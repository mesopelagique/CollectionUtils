...
$result:=$col.map("c_formula_this";Formula:C1597(This:C1470.value+1))  // ++
$expected:=New collection:C1472(2;3;4;5)
ASSERT:C1129($result.equal($expected);"increment failed "+JSON Stringify:C1217($result))

$col:=New collection:C1472(1;2;3;4)
$result:=$col.map("c_formula_this";Formula:C1597(This:C1470.value+$1);2)  // + increment of 2
$expected:=New collection:C1472(3;4;5;6)
ASSERT:C1129($result.equal($expected);"increment failed "+JSON Stringify:C1217($result))

$col:=New collection:C1472(1;2;3;4)
$result:=$col.map("c_formula_this";Formula:C1597(This:C1470.value+$1+$2);2;3)  // + increment of 5 
